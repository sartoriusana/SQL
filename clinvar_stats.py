...
#This script generates a citations table to store variant statistics from ClinVar.
...
#Previamente cargamos los ficheros necesarios 
CLINVAR_STATS = [
    '''
...
def open_clinvar_db(db_file):
    ...
    try:
        ...
        for tableDecl in CLINVAR_STATS:
...
def store_clinvar_file(db, clinvarstats_file):
    with open(clinvarstats_file, "rt", encoding="utf-8") as cf:
        headerMapping = None
        cur = db.cursor()
        with db:
            for line in cf:
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: {0} {{db_file}} {{clinvarstats_file}}".format(sys.argv[0]), file=sys.stderr)
        sys.exit(1)
    db_file = sys.argv[1]
    clinvarstats_file = sys.argv[2]
    db = open_clinvar_db(db_file)
    try:
        store_clinvar_file(db, clinvarstats_file)
    finally:
        db.close()
